...
const AUTO_GENERATED_FILES = ['**/*.d.ts'];
// const E2E_FILES = ['e2e/**/*.{test,spec}.{js,ts,jsx,tsx}'];
// const UNIT_TEST_FILES = ['src/**/__tests__/*'];
const GLOBAL_IGNORES = ['**/dist/**', '**/dist-ssr/**', '**/coverage/**', '**/src/shader/**'];
...
// ESLint comments rules are not relevant to generated files, and they can cause noise and confusion when they trigger in generated files. For example, `@ts-ignore` is often necessary in generated declaration files, but it would trigger `eslint-comments/require-description` if it were enabled.
const eslintCommentsRuleNames = new Set([
  ...Object.keys(comments.recommended.rules ?? {}),
  '@eslint-community/eslint-comments/require-description'
]);

// Disable all ESLint comments rules for generated files.
const disableEslintCommentsRulesForGenerated: Linter.Config['rules'] = Object.fromEntries(
  [...eslintCommentsRuleNames].map(ruleName => [ruleName, 'off'])
);
...
export default defineConfigWithVueTs(
  ...
  */
  {
    // 自動生成ファイルに説教しても誰も幸せにならない。
    // Nobody benefits from lecturing generated files.
    name: 'generated/disable-eslint-comments',
    files: AUTO_GENERATED_FILES,
    rules: disableEslintCommentsRulesForGenerated
  },
